...
CREATE TABLE IF NOT EXISTS `studiostare`.`role` (
  ...
  `role_description` VARCHAR(1024) NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `studiostare`.`user` (
  ...
  `password` VARCHAR(32) NULL COMMENT '密码，非必填，管理员必填。',
  `role_id` INT NULL COMMENT '所属角色，1：管理员；2：抓取新闻的用户；3：关于我们中的人员列表',
  `capture_news_flag` VARCHAR(45) NULL COMMENT '是否需要抓取新闻的标识',
  `twitter_user` VARCHAR(64) NULL COMMENT '抓取新闻的账号',
